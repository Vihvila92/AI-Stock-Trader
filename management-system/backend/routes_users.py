...
from sqlalchemy import select, delete, asc, update
...
SECRET_KEY = "supersecretkey"  # Change to .env or key management in production
ALGORITHM = "HS256"
...
async def get_current_user(token: str = Depends(oauth2_scheme)):
    ...
    async with AsyncSession(engine) as session:
        ...
        if hasattr(user, 'permissions') and isinstance(user.permissions, str):
            try:
                perms = json.loads(user.permissions) if user.permissions not in (None, '', 'null') else {}
            except Exception:
                perms = {}
    ...
    async with AsyncSession(engine) as session:
        ...
        for u in users:
            ...
            if hasattr(u, 'permissions') and isinstance(u.permissions, str):
                try:
                    perms = json.loads(u.permissions) if u.permissions not in (None, '', 'null') else {}
                except Exception:
                    perms = {}
...
async def create_user(user: UserCreate, request: Request):
    ...
    async with AsyncSession(engine) as session:
        ...
        if user.username == 'admin':
            if len(all_users) > 0:
                print(f"[AUDIT] {client_ip} tried to create admin when users already exist -> denied")
                raise HTTPException(status_code=403, detail="Admin can only be created as the first user")
            ...
            if not admin_exists:
                print(f"[AUDIT] {client_ip} tried to create non-admin user before admin exists -> denied")
                raise HTTPException(status_code=403, detail="Admin must be created first")
        # Check if admin already exists when trying to create admin
        if user.username == 'admin':
            result = await session.execute(select(User).where(User.username == 'admin'))
            existing_admin = result.scalars().one_or_none()
            if existing_admin is not None:
                print(f"[AUDIT] {client_ip} tried to create a second admin account -> denied")
                raise HTTPException(status_code=403, detail="Admin account already exists")
        # Prevent admin-only permissions for non-admins
        if user.username != 'admin' and any(k in ADMIN_ONLY_PERMISSIONS and v for k, v in (user.permissions or {}).items()):
            print(f"[AUDIT] {client_ip} tried to create user {user.username} with admin-only permissions -> denied")
            raise HTTPException(status_code=403, detail="You cannot create a user with admin-only permissions")
        ...
        new_user = User(username=user.username, hashed_password=hashed_pw, is_active=1, permissions=permissions_str)
...
async def update_user(user_id: int, data: UserUpdate, request: Request, user=Depends(lambda: require_permission("can_manage_users"))):
    async with AsyncSession(engine) as session:
        ...
        if getattr(user, 'username', None) == 'admin':
            if data.username and data.username != 'admin':
                ...
                raise HTTPException(status_code=403, detail="You cannot change the admin username")
            if data.permissions is not None:
                print(f"[AUDIT] {client_ip} tried to change admin's permissions -> denied")
                raise HTTPException(status_code=403, detail="You cannot change admin permissions")
            if data.password:
                setattr(user, 'hashed_password', bcrypt.hash(data.password))
                print(f"[AUDIT] {client_ip} changed admin's password")
            await session.commit()
            return {"id": user.id, "username": user.username, "permissions": ALL_PERMISSIONS}
        # Prevent admin-only permissions for non-admins
        if data.permissions:
            for k, v in data.permissions.items():
                if k in ADMIN_ONLY_PERMISSIONS and v is True:
                    print(f"[AUDIT] {client_ip} tried to give admin-only permission ({k}) to user {user.username} -> denied")
                    raise HTTPException(status_code=403, detail="You cannot grant admin-only permissions")
        # Update other fields
        if data.username:
            if data.username == 'admin':
                print(f"[AUDIT] {client_ip} tried to rename user {user.username} to admin -> denied")
                raise HTTPException(status_code=403, detail="You cannot rename a user to admin")
...
async def delete_user(user_id: int):
    async with AsyncSession(engine) as session:
        ...
        if first_user is not None and hasattr(first_user, 'id') and isinstance(first_user.id, int):
            if user_id == first_user.id:
                raise HTTPException(status_code=403, detail="You cannot delete the first admin account.")
        if user is None:
            raise HTTPException(status_code=404, detail="User not found")
