"""
Add explicit metadata columns to settings table (category, type, enum, default_value, is_editable)
"""
...
def upgrade():
    ...
    op.add_column('settings', sa.Column('is_editable', sa.Boolean(), nullable=True, server_default='true'))
    ...
    for row in result:
        try:
            meta = json.loads(row.value) if row.value and row.value.strip().startswith('{') else None
        except Exception:
            meta = None
        if meta and isinstance(meta, dict):
            ...
            conn.execute(sa.text("""
                UPDATE settings SET category=:category, type=:typ, enum=:enum, default_value=:default_value WHERE id=:id
            """), dict(category=category, typ=typ, enum=enum, default_value=default_value, id=row.id))
