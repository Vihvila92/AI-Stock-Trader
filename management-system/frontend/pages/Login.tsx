...
export default function Login({ onLogin }: Props) {
  ...
  const loginBg = getAppearanceValue(appearance, 'login_background_color', '#f9fafb');
  const boxBg = getAppearanceValue(appearance, 'login_box_color', '#fff');
  const textColor = getAppearanceValue(appearance, 'login_text_color', '#111827');
  ...
  return (
    <div style={{ minHeight: '100vh', background: loginBg, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <main
        ...
        style={{ background: boxBg, color: textColor, width: 400, boxShadow: '0 2px 16px 0 #0001', borderRadius: 12 }}
      >
        <div style={{ textAlign: 'center', marginBottom: 24 }}>
          {isValidLogo ? (
            <img src={logoUrl} alt="Logo" style={{ maxWidth: 120, maxHeight: 80, margin: '0 auto' }} />
          ) : (
            <span style={{ fontWeight: 700, fontSize: 28 }}>{siteName}</span>
          )}
        </div>
        <form onSubmit={handleSubmit} style={{ maxWidth: 320, margin: 'auto', padding: 0 }}>
          <h2 style={{ textAlign: 'center', fontWeight: 700, fontSize: 24, marginBottom: 24 }}>Login</h2>
          <div style={{ marginBottom: 20 }}>
            ...
            <input
              ...
              required
              style={{ width: '100%', padding: 10, borderRadius: 6, border: '1px solid #d1d5db', background: '#f3f4f6', color: textColor, fontSize: 16, outline: 'none' }}
            />
          </div>
          <div style={{ marginBottom: 20 }}>
          ...
          {error && <div style={{ color: 'red', marginTop: 12, marginBottom: 8 }}>{error}</div>}
          <button
            type="submit"
            disabled={loading}
            style={{ marginTop: 8, width: '100%', background: '#2563eb', color: '#fff', border: 'none', borderRadius: 6, padding: 12, fontWeight: 600, fontSize: 16, boxShadow: '0 1px 4px #0001', cursor: loading ? 'not-allowed' : 'pointer' }}
          >
