...
function formatCategoryName(cat: string) {
  ...
  words = words.map(w => {
    if (["api", "id", "url", "ip"].includes(w.toLowerCase())) return w.toUpperCase();
...
const CATEGORY_ORDER = ["general", "appearance", "trading", "notifications", "security", "advanced"];
...
export default function Settings() {
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      ...
      if (res.ok) {
        ...
        const categories = Array.from(new Set(data.map((s: Setting) => s.category || "general")));
  ...
  let categories = Array.from(new Set(settings.map(s => String(s.category ?? "")))).filter(Boolean);
  ...
  const shownTab = categories.includes(activeTab) ? activeTab : (categories[0] || "");
  ...
  const handleSave = async () => {
    ...
    setSaveStatus("");
    const token = authHeader();
    const tabSettings = settings.filter(s => (s.category || "general") === activeTab);
    let success = true;
    for (const setting of tabSettings) {
      if (edited[setting.key] !== undefined && edited[setting.key] !== setting.value) {
    ...
    setSaving(false);
    setSaveStatus(success ? "Settings saved!" : "Failed to save some settings.");
  ...
  return (
    <DashboardLayout>
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        {/* Tab content */}
        <div>
          {settings.filter(s => String(s.category) === shownTab).map(setting => {
            // Enum-tyyppinen asetus
            const enumOptions = parseEnum(setting.enum);
            if (setting.type === 'enum' && enumOptions.length > 0) {
              const lang = typeof navigator !== 'undefined' && navigator.language?.startsWith('fi') ? 'fi' : 'en';
              return (
                <div key={setting.key} className="mb-6">
                  <label className="font-semibold block mb-1" htmlFor={setting.key}>{setting.label || setting.key}</label>
                  <select
                    id={setting.key}
            ...
            if (setting.key === 'logo_url') {
              ...
              return (
                <div key={setting.key} className="mb-6">
                  ...
                  <div>
                    {logoUrl ? (
                      <div className="mb-2"><img src={logoUrl} alt="Logo" style={{ maxWidth: 120, maxHeight: 80, background: '#fff', border: '1px solid #eee', padding: 4 }} /></div>
                    ) : null}
                    <input
                      id={setting.key}
                      type="text"
                      className="border rounded px-3 py-2 w-full text-gray-800 mb-2"
                      value={edited[setting.key] !== undefined ? edited[setting.key] : (() => {
                        try {
                          const parsed = JSON.parse(setting.value);
                          return parsed.value !== undefined ? String(parsed.value) : setting.value;
                    ...
                    <input
                      ...
                      onChange={async e => {
                        ...
                        const filename = `/logo_uploaded_${Date.now()}.${file.name.split('.').pop()}`;
                        try {
                    ...
                    />
                    <div className="text-xs text-gray-500 mt-1">Voit antaa suoran url:n tai ladata logon (vain dev-ympäristössä).</div>
                  </div>
                </div>
              );
            }
